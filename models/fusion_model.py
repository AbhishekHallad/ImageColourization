...
class FusionModel(BaseModel):
    ...
    def save_current_imgs(self, path):
        ...
        # Priority 1: Luminance-aware saturation boost (midtones only, not highlights/shadows)
        from skimage import color as skcolor
        hsv = skcolor.rgb2hsv(out_img_np)
        
        # Get luminance (V channel in HSV, or convert from RGB)
        L_channel = self.full_real_A.cpu().data.numpy()[0, 0, :, :]
        # Normalize L to 0-1 range (it's already normalized from the model)
        L_normalized = (L_channel - L_channel.min()) / (L_channel.max() - L_channel.min() + 1e-6)
        
        # Create midtone mask: boost saturation only in midtones (0.2-0.8 range)
        # Use smooth falloff to avoid hard edges
        midtone_mask = np.clip((L_normalized - 0.2) / 0.3, 0, 1.0) * np.clip((0.8 - L_normalized) / 0.3, 0, 1.0)
        midtone_mask = np.clip(midtone_mask * 2.0, 0, 1.0)  # Smooth bell curve
        
        # Apply moderate saturation boost (1.2x) to midtones only
        # Highlights and shadows get minimal boost (1.05x) to preserve detail
        saturation_boost = 1.05 + (1.2 - 1.05) * midtone_mask
